...
void adcRecord::adcRecord_rxMsgProcess(quint8 *rxData)
{
    ...
    else if (_pRxHead->cmdId == _cmdId_flashRecordDatas)
    {
        ...
        {
            if (((_prxData->adc_ch_Enable >> 0) & 0x01) == 1) {
                quint16 val = SWAP16(_prxData->adcData[dataIndex]);
                dataIndex++;
                adcRecord1.append(val * 5.0 / 32768.0 - 5.0);
            }
            if (((_prxData->adc_ch_Enable >> 1) & 0x01) == 1) {
                quint16 val = SWAP16(_prxData->adcData[dataIndex]);
                dataIndex++;
                adcRecord2.append(val * 5.0 / 32768.0 - 5.0);
            }
            if (((_prxData->adc_ch_Enable >> 2) & 0x01) == 1) {
                quint16 val = SWAP16(_prxData->adcData[dataIndex]);
                dataIndex++;
                adcRecord3.append(val * 5.0 / 32768.0 - 5.0);
            }
            if (((_prxData->adc_ch_Enable >> 3) & 0x01) == 1) {
                quint16 val = SWAP16(_prxData->adcData[dataIndex]);
                dataIndex++;
                adcRecord4.append(val * 5.0 / 32768.0 - 5.0);
            }
            if (((_prxData->adc_ch_Enable >> 4) & 0x01) == 1) {
                quint16 val = SWAP16(_prxData->adcData[dataIndex]);
                dataIndex++;
                adcRecord5.append(val * 5.0 / 32768.0 - 5.0);
            }
            if (((_prxData->adc_ch_Enable >> 5) & 0x01) == 1) {
                quint16 val = SWAP16(_prxData->adcData[dataIndex]);
                dataIndex++;
                adcRecord6.append(val * 5.0 / 32768.0 - 5.0);
            }
            if (((_prxData->adc_ch_Enable >> 6) & 0x01) == 1) {
                quint16 val = SWAP16(_prxData->adcData[dataIndex]);
                dataIndex++;
                adcRecord7.append(val * 5.0 / 32768.0 - 5.0);
            }
            if (((_prxData->adc_ch_Enable >> 7) & 0x01) == 1) {
                quint16 val = SWAP16(_prxData->adcData[dataIndex]);
                dataIndex++;
                adcRecord8.append(val * 5.0 / 32768.0 - 5.0);
            }
